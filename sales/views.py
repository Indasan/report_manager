...
def home_view(request):
    ...
    if request.method == 'POST':
        ...
        sale_qs = Sale.objects.filter(created__range=(date_from, date_to))
        if len(sale_qs) > 0:
            sales_df = pd.DataFrame(sale_qs.values())
            #Применяя функции заменяем значения, вместо id получаем имена
            ...
            #Переименовываем наименования столбцов
            ...
            #Объединяем данные двух таблиц по полю sales_id
            merged_df = pd.merge(sales_df, positions_df, on='sales_id')

            #Объединяем данные по номеру транзакции и суммируем стоимости позиций
            ...
            no_data = 'Нет данных на выбранный период времени'
